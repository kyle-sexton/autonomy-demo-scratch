#!/usr/bin/env bash
# Corpus-wide heading-cite resolver (markdown-debt lane B). Verifies every
# `file.md` "Anchor" prose cite in the durable corpus against the actual anchor
# vocabulary of the cited file: markdown headings (h1-h6) plus bold lead-ins
# (bullet `- **Name**`, paragraph `**Name**`, table-cell `| **Name**`).
# Dependency-free commit path: one POSIX-awk process (no gawk extensions — runs
# on gawk, mawk, BSD awk); file contents are read via getline so no per-file
# forks and no ARGV-length ceiling. Exit 0 clean, 1 on unresolved cites, 2 usage.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Corpus-scope SSOT — NOISE_RE shared via corpus-scope.sh. The cite corpus
# is ALL tracked md minus .work/ + NOISE_RE + vendored dirs — intentionally
# broader than SCOPE_RE (dead pointers are debt anywhere tracked).
# shellcheck source=corpus-scope.sh
source "${SCRIPT_DIR}/corpus-scope.sh"

VENDOR_RE='(^|/)vendor/'

# Immutable historical run records — correct at write time; later renames must
# not demand rewriting history (user-approved Phase 2 scope decision).
RUN_RECORD_RE='^(\.claude/routines/runs/|automations/runs/|\.cursor/automations/[^/]+/(runs|findings)/)'
...
source-dir-relative, then unique suffix match corpus-wide. Historical run
records (.claude/routines/runs/, automations/runs/, .cursor/automations/*/{runs,findings}/)
are excluded from the corpus.

Options:
  --root <dir>           Repo root to enumerate + resolve against (default: this script's git toplevel).
...
main() {
  ...
  if [[ -z "$corpus_file" ]]; then
    git -C "$root" ls-files '*.md' | tr -d '\r' \
      | grep -vE "$NOISE_RE" | grep -v '^\.work/' | grep -vE "$VENDOR_RE" \
      | grep -vE "$RUN_RECORD_RE" \
